...
public class CMCommentController {
    ...
    public String listCommentsByUser(@PathVariable Long userId, Model model) {
        logger.info("Requesting comments by user: User ID {}", userId);
        List<CMComment> comments = cmCommentService.findCommentsByUserId(userId);
        model.addAttribute("comments", comments);
        logger.info("Comments by user retrieved successfully: User ID {}", userId);
        return "userCommentList"; // 댓글 리스트를 보여줄 뷰 이름으로 수정 필요
    }
}
